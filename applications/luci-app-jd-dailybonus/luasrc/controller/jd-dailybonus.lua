...
function index()
    ...
    entry({'admin', 'robot', 'jd-dailybonus'}, alias('admin', 'services', 'jd-dailybonus', 'client'), _('京东签到服务'), 10).dependent = true -- 首页
    entry({'admin', 'robot', 'jd-dailybonus', 'client'}, cbi('jd-dailybonus/client', {hidesavebtn = true, hideresetbtn = true}), _('客户端'), 10).leaf = true -- 基本设置
    entry({'admin', 'robot', 'jd-dailybonus', 'log'}, form('jd-dailybonus/log'), _('日志'), 30).leaf = true -- 日志页面
    entry({'admin', 'robot', 'jd-dailybonus', 'script'}, form('jd-dailybonus/script'), _('脚本查看'), 20).leaf = true -- 直接配置脚本
    entry({'admin', 'robot', 'jd-dailybonus', 'run'}, call('run')) -- 执行程序
    entry({'admin', 'robot', 'jd-dailybonus', 'update'}, call('update')) -- 执行更新
    entry({'admin', 'robot', 'jd-dailybonus', 'check_update'}, call('check_update')) -- 检查更新
    entry({'admin', 'robot', 'jd-dailybonus', 'qrcode'}, call('qrcode')) -- 获取二维码
    entry({'admin', 'robot', 'jd-dailybonus', 'check_login'}, call('check_login')) -- 获取二维码
    entry({'admin', 'robot', 'jd-dailybonus', 'realtime_log'}, call('get_log')) -- 获取实时日志
end
